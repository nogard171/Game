...
//		Renderer.renderQuad(chatBounds, new Color(0, 0, 0, 0.5f));
		...
		for (int i = startIndex; i > endIndex; i--) {
			...
			for (int y = temp - 1; y >= 0; y--) {
				int endLine = ((y + 1) * maxLength > lineString.length() ? lineString.length() : (y + 1) * maxLength);

				String line = lineString.substring(y * maxLength, endLine);

				Renderer.renderText(new Vector2f(position.x + 5, ((position.y - 29 + (c * 12))) - ((yOffset - 1) * 12)),
						line, 12, Color.white);
				yOffset++;
				currentC++;
			}
